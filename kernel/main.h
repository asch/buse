...
struct buse_blkdev
{
	...
	/* Flag which is set once the device is created. This is important
	 * because we don't create device immediately but wait until all
	 * control queues are connected. Hence it is important to keep track of
	 * it to know whether to destroy the block device during shut down.
	 */
	bool created;
};
...
struct buse
{
	...
	/* Sequential numbers for writes. We define one counter per collision
	 * domain to avoid excessive cache coherency protocol traffic. This
	 * creates ordering on all writes inside the collision domain which is
	 * enough. A counter per sector would be optimal, but memory
	 * inefficient. One counter per whole address space would be to
	 * contended. Collision domains are a good compromise.
	 */
	atomic_t *collision_counters;

	/* Individual queues structure related to the created character
	 * devices. */
	...
	/* Index of the created block device corresponding to the created
	 * configfs node with mkdir. */
	u64 index;
	...
	/* Size of the shared memory between kernel and userspace which is used
	 * for sending write chunks to the userspace. This is per one write
	 * queue.
	 */
	u64 write_shm_size;

	/* Size of the shared memory between kernel and userspace which is used
	 * for sending individual reads to the userspace. This is per one write
	 * queue. Compared to writes reads are not batched into chunks. Each
	 * individual read is sent to userspace.
	 */
	u64 read_shm_size;
	...
	/* Number of hw queues block device provides. Usually number of CPUs is
	 * the right value. */
	u64 hw_queues;
...
struct buse_rqueue
{
	...
	/* Flag whether individual queue is bound, i.e. the character device is
	 * opened and mmaped. */
	atomic_t bound;

	/* Mapping from the bitmap index to the read chunk. Used when bitmap
	 * index is acknowledged to know what read to acknowledge.
	 */
	...
	/* Queue with chunks already sent to user space. Important when user
	 * space side crashes to rerun not acknowledged but fetched reads
	 * again.
	 */
	struct list_head fetched_chunks;

	/* If true the termination chunk was already sent to user space and no
	 * other chunk can be processed by the other end of the queue.
	 */
	bool terminated;
};
...
struct write_chunk
{
	...
	/* List of all reads waiting for any write in the chunk. These reads
	 * are postponed and woken up when the write is acknowledged. Solution
	 * of the read after write hazard.
	 */
	struct list_head dependent_reads;

	/* If the chunks is flush chunk, i.e. just performing the flush
	 * operation, we store the cmd pointer here to be able to acknowledge
	 * it easily.
	 */
	struct buse_cmd *cmd;
};
...
struct buse_wqueue
{
	...
	/* Queue with chunks already sent to user space. Important when user
	 * space side crashes to rerun not acknowledged but fetched writes
	 * again.
	 */
	struct list_head fetched_chunks;
...
struct buse_cmd
{
	...
	/* True if some operation failed and at the end the cmd should be
	 * canceled and report it to the blk-mq.
	 */
	bool canceled;
	...
	union {
		struct {
			/* How many more queues need to do their check for read
			 * after write hazard.
			 */
			atomic_t queues_pending;

			/* How many writes need to be acknowledged until the
			 * read can be send to user space.
			 */
			atomic_t write_deps;
		} read;

		struct {
			/* How many more queues need to send the flush chunk.
			 * This is used when broadcasting flush command.
			 */
			atomic_t queues_pending;
		} flush;
	};
...
/* Checks if all queues are connected and if they are it creates the block
 * device and is ready to serve io commands.
 */
